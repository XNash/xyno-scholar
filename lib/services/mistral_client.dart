...
class MistralApiException implements Exception {
  final String message;
  const MistralApiException(this.message);
  @override
  String toString() => message;
}

class MistralClient {
  /// Requests go through a minimal Cloudflare Worker relay
  /// (`xyno-scholar-relay`, see /worker) rather than directly to
  /// `api.mistral.ai`, because Mistral's `chat/completions` endpoint
...
''';
  ...
  Future<Map<String, dynamic>> _chat({
    ...
    try {
      ...
      throw const MistralApiException(
        'Received an unreadable response from Mistral.',
      );
    }
    return decoded;
  }
  ...
  /// the model's first output is not valid JSON. `response_format:
  /// json_object` only guarantees syntactically valid JSON, not that it
  /// matches our exact schema, so this stays in place even with JSON mode
  /// enabled.
  Future<Map<String, dynamic>> _requestJson({
    ...
    try {
      ...
      try {
        ...
        throw const MistralApiException(
          'Mistral did not return valid JSON, even after a retry.',
        );
      }
    }
  }
