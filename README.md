...
The app calls the Cerebras Inference API (`https://api.cerebras.ai/v1/chat/completions`,
OpenAI-compatible) directly from the browser using a Cerebras API key entered
...
- A Cerebras API key (entered in the app, not configured at build time)
...
"Enter your Cerebras API key" screen — paste your key there to unlock the app.
...
## Notes on the Cerebras API and CORS

This app calls Cerebras directly from the browser with no proxy. If Cerebras
ever changes its CORS policy for browser-origin requests, direct calls from
a deployed static site could start failing with a CORS error in the browser
console — in that case, a minimal CORS-passthrough function (which does not
need to hold any secret; the user's key still travels from the client on
every request) would need to sit in front of the API endpoint.
...
lib/
  ...
  services/                  # Cerebras client, sanitization, field detection, storage, BibTeX
